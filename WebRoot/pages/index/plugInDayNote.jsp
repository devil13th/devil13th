...
<div class="portlet portlet-sortable box blue-hoki" data-step="7" data-intro="系统记事">
	<div class="portlet-title">
		<div class="caption">
			<i class="glyphicon glyphicon-list-alt"></i> <s:text name="common.projectNote"></s:text> </div>
		<div class="actions">
			<a href="javascript:;" class="btn btn-default btn-sm" onclick="showWin('${ctx}/se/se!seDayNoteTimeLine.do?projectId=${projectId }')">
					<i class="glyphicon glyphicon-time" ></i> Time Line
			</a>
			<a href="javascript:;" class="btn btn-default btn-sm" onclick="showWin('${ctx}/se/se!seDayNoteList.do')">
					<i class="glyphicon glyphicon-th-list" ></i> More List
			</a>
			<a href="javascript:;" class="btn btn-default btn-sm" onclick="newDayNote()">
	...
	<div class="portlet-body portlet-body-self">
			...
			<div>
			    <div class="input-group">
			      <input type="text" class="form-control" id="pluginDayNoteQueryKey" placeholder="Search for...">
			      <span class="input-group-btn">
			        <button class="btn btn-default" type="button" onclick="reloaddayNoteTab()"> <i class="glyphicon glyphicon-search"></i> Search</button>
			      </span>
			    </div><!-- /input-group -->
			</div>
		
	<table id="dayNoteTab" class="display compact" cellspacing="0" width="100%">
	<thead>
	<tr>
...
var dayNoteTab;
$(function(){
	dayNoteTab =  $('#dayNoteTab').DataTable( {
		/*
		dom属性	字母含义
		...
		*/
	    ...
	    ajax: {
	    	//后台获取数据地址
	        url:"${ctx}/index/index!plugInDayNoteGetData.do",
	        //请求类型
	        type: "POST",
	        data:function(d){
	           ...
	           //设置查询参数
	        	...
	        	d.NOTE_TITLE = $("#pluginDayNoteQueryKey").val();
	            return d;
	        },
	        //错误处理
	...
	            {name:"Title",data: "NOTE_TITLE",defaultContent:"" ,orderable:false},
				{name:"Project",title:"Project",data: "PROJECT_ID",defaultContent:"" ,orderable:false}
	        ]
	});
	
	$('#dayNoteTab tbody').on('click', 'td', function(){
		var data = dayNoteTab.row(this).data();
		console.log(data);
		//var url = "${ctx}/"+data.FORMKEY+"?jobno=" + data.BUSSINESSKEY + "&taskId=" + data.TASKID + "&procInsId=" + data.PROCINSID;
		//openLayer(data.WORKNAME,url);
		openLayer("项目记事","${ctx}/se/se!seDayNoteForm.do?seDayNote.noteId="+data.NOTE_ID);
	});
})


function newDayNote(){
	openLayer("项目记事","${ctx}/se/se!seDayNoteForm.do?seDayNote.projectId=${projectId}");
}

//刷新
function reloaddayNoteTab(){
		...
		*/
		dayNoteTab.ajax.reload(null,false);
}
</script>