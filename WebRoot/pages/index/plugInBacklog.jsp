...
<div class="portlet portlet-sortable box green-meadow" data-step="12" data-intro="<s:text name="common.backlog"></s:text>">
	<div class="portlet-title">
		<div class="caption">
			<i class="glyphicon glyphicon-list-alt"></i> <s:text name="common.backlog"></s:text> </div>
		<div class="actions">
			<a href="javascript:;" class="btn btn-default btn-sm" onclick="showWin('${ctx}/backlog/backlog!backlog.do')">
					<i class="glyphicon glyphicon-th-list" ></i> More List
			</a>
			<a href="javascript:;" class="btn btn-default btn-sm" onclick="newBacklog()">
	...
	<div class="portlet-body portlet-body-self">
			...
			<div>
			    <div class="input-group">
			      <input type="text" class="form-control" id="pluginBacklogQueryKey" placeholder="Search for...">
			      <span class="input-group-btn">
			        <button class="btn btn-default" type="button" onclick="reloadBacklogTab()"> <i class="glyphicon glyphicon-search"></i> Search</button>
			      </span>
			    </div><!-- /input-group -->
			</div>
		
	<table id="backlogTab" class="display compact" cellspacing="0" width="100%">
	</table>
	</div>
</div>
...
$(function(){
	backlogTab =  $('#backlogTab').DataTable( {
		...
		*/
	    ...
	    ajax: {
	    	//后台获取数据地址
	        url:"${ctx}/index/index!plugInBacklogGetData.do",
	        //请求类型
	        type: "POST",
	        data:function(d){
	           ...
	           //设置查询参数
	        	...
	        	d.blContent = $("#pluginBacklogQueryKey").val();
	        	d.blStatus = '1';
	            return d;
	        },
	        //错误处理
	...
	        columns: [
	            {name:"Module",title:"Module",data: "blContent",width:'60%',defaultContent:"" ,orderable:false},
				{name:"User",title:"User",data: "blExeUserName",width:'20%',defaultContent:"" ,orderable:false},
				{name:"Project",title:"Project",data: "blSys",width:'20%',defaultContent:"" ,orderable:false}
	        ]
	});
	
	$('#backlogTab tbody').on('click', 'td', function(){
		var data = backlogTab.row(this).data();
		console.log(data);
		//var url = "${ctx}/"+data.FORMKEY+"?jobno=" + data.BUSSINESSKEY + "&taskId=" + data.TASKID + "&procInsId=" + data.PROCINSID;
		//openLayer(data.WORKNAME,url);
		openLayer("公共模块","${ctx}/backlog/backlog!backlogInfoForm.do?bli.blId="+data.blId);
	});
})


function newBacklog(){
	openLayer("公共模块","${ctx}/backlog/backlog!backlogInfoForm.do");
}

//刷新
function reloadBacklogTab(){
		...
		*/
		backlogTab.ajax.reload(null,false);
}
</script>